...
class GoodsReceiptListModelViewTest(APITestCase):
	...
	def test_get_valid_goods_receipt_list(self):
		# Getting goods receipt list with invoice id 1
		response = self.client.get(reverse("goods_receipt:get_goods_receipt_list", args=[1]))
		self.assertEqual(response.status_code, 200)
		# Expects one goods receipt 
		self.assertEqual(len(response.data), 1)
		data = response.data[0]["goods_receipt_info"]
		self.assertEqual(data["goods_receipt_entry_id"], 1)
		self.assertEqual(data["invoice_line_item_id"], 1)
		self.assertEqual(data["buyer_goods_receipt_id"], "12")
		self.assertEqual(Decimal(data["delivered_quantity"]), Decimal("30"))
		self.assertEqual(Decimal(data["receipt_quantity_rejected"]), Decimal("10"))
		self.assertEqual(Decimal(data["receipt_quantity_accepted"]), Decimal("20"))

	def test_get_invalid_goods_receipt_list(self):
		# Getting goods receipt with invoice id 2 which does not exist
		response = self.client.get(reverse("goods_receipt:get_goods_receipt_list", args=[2]))
		self.assertEqual(response.status_code, 404)
	
	def test_post_valid_goods_receipt_list(self):
		# Making a new invoice to post a goods receipt
		data_invoice_info = {
			"invoice_type": "goods",
			"created_by_user_id": self.user1,
			"seller_invoice_id": "123",
		...
		self.invoice = Invoice.objects.create(**data_invoice_info)
		data_invoice_item = {
			"invoice_id" : self.invoice,
			"purchase_order_line_item_id": self.purchase_order_item,
			"item_id": self.item,
		...
		self.invoice_item = InvoiceItem.objects.create(**data_invoice_item)
		data_invoice_item_charge_1 = {
			"invoice_line_item_id" : self.invoice_item,
			"charge_name": "GST",
			"charge_percentage": 5
		...
		response = self.client.post(reverse("goods_receipt:post_goods_receipt_list", args=[1,2]), json.dumps(data), content_type="application/json")
		self.assertEqual(response.status_code, 201)

		data = GoodsReceiptSerializer(GoodsReceipt.objects.get(goods_receipt_entry_id=2)).data
		self.assertEqual(data["goods_receipt_entry_id"], 2)
		self.assertEqual(data["invoice_line_item_id"], 2)
		self.assertEqual(data["buyer_goods_receipt_id"], "13")
		self.assertEqual(Decimal(data["delivered_quantity"]), Decimal("30"))
		self.assertEqual(Decimal(data["receipt_quantity_rejected"]), Decimal("10"))
		self.assertEqual(Decimal(data["receipt_quantity_accepted"]), Decimal("20"))

		# Checking the invoice if the status and amount due has been updated or not
		invoice_line_item = InvoiceItem.objects.get(invoice_line_item_id=2)
		...
		# Checking amount_due and invoice status
		self.assertEqual(Decimal(accepted_amount), round(Decimal(invoice_line_item.amount_due), DECIMAL_COMPARISON_PRECISION))
		self.assertEqual(invoice.status, "ongoing")
	
	def test_test_post_invalid_goods_receipt_list(self):
		# Making a new invoice to post a goods receipt
		data_invoice_info = {
			"invoice_type": "goods",
			"created_by_user_id": self.user1,
		...
		# Posting goods_receipt with user_id 3 which does not exist
		response = self.client.post(reverse("goods_receipt:post_goods_receipt_list", args=[3, 2]), json.dumps(data), content_type="application/json")
		self.assertEqual(response.status_code, 404)
		...
		# Posting goods_receipt with invoice_line_item_id 3 which does not exist
		response = self.client.post(reverse("goods_receipt:post_goods_receipt_list", args=[1, 3]), json.dumps(data), content_type="application/json")
		self.assertEqual(response.status_code, 404)
		...
		# Posting goods_receipt with data format which is wrong
		response = self.client.post(reverse("goods_receipt:post_goods_receipt_list", args=[1, 2]), json.dumps(data), content_type="application/json")
		...
		data = {
			"goods_receipt_info": {
				...
				"receiving_user_email": "matt@factwise.io",
				"measurement_unit_id": 1,
				"delivered_quantity": 160,
				"problem_category": "Faulty Quality",
				"receipt_quantity_accepted": 131,
				"receipt_quantity_rejected": 29
			}
		}
		# Posting goods_receipt with more accepted quantity than max pending quantity and that is 130 here.
		response = self.client.post(reverse("goods_receipt:post_goods_receipt_list", args=[1, 2]), json.dumps(data), content_type="application/json")
		self.assertEqual(response.status_code, 400)