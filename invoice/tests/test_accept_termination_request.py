...
class InvoiceTerminationModelViewTest(TestCase):
    ...
    def test_patch_valid_rescind_invoice_by_seller(self):    
        '''
        patchting invoice with user id  2  and invoice id 1
        '''
        invoice = Invoice.objects.get(invoice_id=1)
        invoice.status = "issued"
        invoice.save()
        response = self.client.patch(reverse("invoice:rescind_invoice" , args=[2,1]))
        ...
        self.assertEqual(response.data["invoice_info"]["status"], "rescind")


    def test_patch_invalid_rescind_invoice(self):
        invoice = Invoice.objects.get(invoice_id=1)
        invoice.status = "issued"
        invoice.save()
        '''
        patchting invoice with user id  1 and invoice id 1(no relation between user and invoice)
        '''
        response = self.client.patch(reverse("invoice:rescind_invoice" , args=[1,1]))
        self.assertEqual(response.status_code, 404)
        '''
        patchting invoice with user id  1 and  invoice id 2(non-existent)
        '''
        response = self.client.patch(reverse("invoice:rescind_invoice" , args=[1,2]))
        self.assertEqual(response.status_code, 404)
        '''
        patchting invoice with user id 3(non-existent) and  invoice id 1
        '''
        response = self.client.patch(reverse("invoice:rescind_invoice" , args=[3,1]))
        self.assertEqual(response.status_code, 404)
    
    def test_patch_valid_invoice_accept_termination_buyer_side(self):
        # Accepting Termination Request from seller
        # Requesting Termination from seller side
        self.invoice.closing_user_id_seller = self.user2
        self.invoice.closing_comment_seller = "Not enough quantity"
        self.invoice.status = "termination_request"
        self.invoice.closing_datetime_seller = timezone.now()
        self.invoice.save()
        # Verifying the current data before making request
        invoice = Invoice.objects.get(invoice_id=1)
        self.assertEqual(invoice.status, "termination_request")
        self.assertIsNotNone(invoice.closing_user_id_seller)
        self.assertEqual(invoice.closing_comment_seller, "Not enough quantity")
        self.assertIsNotNone(invoice.closing_datetime_seller)
        self.assertIsNone(invoice.closing_user_id_buyer)
        self.assertEqual(invoice.closing_comment_buyer, "")
        self.assertIsNone(invoice.closing_datetime_buyer)
        self.assertIsNone(invoice.invoice_close_datetime)


        data = {
            "closing_comment_buyer": "Okay"
        }
        response = self.client.patch(reverse("invoice:invoice_accept_termination", args=[1,1,1]), json.dumps(data), content_type="application/json")
        self.assertEqual(response.status_code, 200)

        invoice = Invoice.objects.get(invoice_id=1)
        self.assertEqual(invoice.status, "closed")
        self.assertEqual(invoice.closing_user_id_buyer.user_id, 1)
        self.assertEqual(invoice.closing_comment_buyer, "Okay")
        self.assertIsNotNone(invoice.closing_datetime_buyer)
        self.assertIsNotNone(invoice.invoice_close_datetime)
    
    def test_patch_valid_invoice_accept_termination_seller_side(self):
        # Accepting Termination Request from buyer
        # Requesting Termination from buyer side
        self.invoice.closing_user_id_buyer = self.user1
        self.invoice.closing_comment_buyer = "Wrong Quantity Mentioned"
        self.invoice.status = "termination_request"
        self.invoice.closing_datetime_buyer = timezone.now()
        self.invoice.save()
        # Verifying the current data before making request
        invoice = Invoice.objects.get(invoice_id=1)
        self.assertEqual(invoice.status, "termination_request")
        self.assertIsNotNone(invoice.closing_user_id_buyer)
        self.assertEqual(invoice.closing_comment_buyer, "Wrong Quantity Mentioned")
        self.assertIsNotNone(invoice.closing_datetime_buyer)
        self.assertIsNone(invoice.closing_user_id_seller)
        self.assertEqual(invoice.closing_comment_seller, "")
        self.assertIsNone(invoice.closing_datetime_seller)
        self.assertIsNone(invoice.invoice_close_datetime)
        ...
        response = self.client.patch(reverse("invoice:invoice_accept_termination", args=[2,3,1]), json.dumps(data), content_type="application/json")
        self.assertEqual(response.status_code, 200)

        invoice = Invoice.objects.get(invoice_id=1)
        self.assertEqual(invoice.status, "closed")
        self.assertEqual(invoice.closing_user_id_seller.user_id, 2)
        self.assertEqual(invoice.closing_comment_seller, "Okay")
        self.assertIsNotNone(invoice.closing_datetime_seller)
        self.assertIsNotNone(invoice.invoice_close_datetime)
    
    def test_patch_invalid_invoice_accept_termination_buyer_side(self):
        # Accepting Termination Request when seller has not requested for termination
        # Verifying the current data before making request
        invoice = Invoice.objects.get(invoice_id=1)
        self.assertEqual(invoice.status, "ongoing")
        self.assertIsNone(invoice.closing_user_id_seller)
        self.assertEqual(invoice.closing_comment_seller, "")
        self.assertIsNone(invoice.closing_datetime_seller)
        self.assertIsNone(invoice.closing_user_id_buyer)
        ...
        # Accepting a termination request if the invoice is already closed
        # First making the invoice closed
        self.invoice.closing_user_id_seller = self.user2
        self.invoice.closing_comment_seller = "Not enough quantity"
        self.invoice.closing_datetime_seller = timezone.now()
        self.invoice.closing_user_id_buyer = self.user1
        self.invoice.closing_comment_buyer = "Okay"
        self.invoice.closing_datetime_buyer = timezone.now()
        self.invoice.invoice_close_datetime = timezone.now()
    ...
    def test_patch_invalid_invoice_accept_termination_seller_side(self):
