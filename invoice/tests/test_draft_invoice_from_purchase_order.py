...
class DraftInvoiceFromPurchaseOrderModelViewTest(TestCase):
	...
	def test_get_valid_draft_invoice_purchase_order(self):
		# Getting draft invoice with purchase order with id 1
		response = self.client.get(reverse("invoice:invoice_from_purchase_order", args=[1]))
		...
		self.assertEqual(invoice_info["buyer_purchase_order_id"], "1233")
		self.assertEqual(invoice_info["buyer_id"], 1)
		self.assertEqual(invoice_info["seller_id"], 3)
		self.assertEqual(invoice_info["status"], "draft")

		self.assertEqual(len(invoice_item_list), 1)
		self.assertEqual(invoice_item_list[0]["item_info"]["purchase_order_line_item_id"], 1)
		...
		self.assertEqual(Decimal(invoice_item_list[0]["item_info"]["shipping_per_unit"]), Decimal("5"))
		self.assertEqual(Decimal(invoice_item_list[0]["item_info"]["quantity_invoiced"]), Decimal("100"))
		...
		# Posting an invoice and then chekcing again, will affect the quantity_invoiced
		data_invoice_info = {
			"invoice_type": "goods",
			"created_by_user_id": self.user1,
		...
		# Again requesting the darft invoice 
		response = self.client.get(reverse("invoice:invoice_from_purchase_order", args=[1]))
		self.assertEqual(response.status_code, 200)
		invoice_info = response.data["invoice_info"]
		invoice_item_list = response.data["invoice_item_list"]
		...
		self.assertEqual(Decimal(invoice_item_list[0]["item_info"]["rate"]), Decimal("100"))
		self.assertEqual(Decimal(invoice_item_list[0]["item_info"]["shipping_per_unit"]), Decimal("5"))
		self.assertEqual(Decimal(invoice_item_list[0]["item_info"]["quantity_invoiced"]), Decimal("70")) # Main info expected
		...
		# Making a Goods Receipt against this invoice and then requesting
		...
		self.assertEqual(Decimal(invoice_item_list[0]["item_info"]["rate"]), Decimal("100"))
		self.assertEqual(Decimal(invoice_item_list[0]["item_info"]["shipping_per_unit"]), Decimal("5"))
		self.assertEqual(Decimal(invoice_item_list[0]["item_info"]["quantity_invoiced"]), Decimal("80")) # Main info expected
		...
		# Quality Check can be also created for further assurance

	def test_get_invalid_draft_invoice_purchase_order(self):
		# Getting draft invoice with purchase order with id 2 which does not exist
		response = self.client.get(reverse("invoice:invoice_from_purchase_order", args=[2]))
