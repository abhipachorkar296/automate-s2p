...
class DraftInvoiceModelViewTest(TestCase):
    ...
    def test_get_valid_draft_invoice_list(self):
        # Getting draft invoice with id 1
        response = self.client.get(reverse("invoice:draft_invoice_list", args=[1,1]))
    ...
    def test_get_invalid_draft_invoice_list(self):
        # Getting draft invoice with user id 3 which doesn't exist
        response = self.client.get(reverse("invoice:draft_invoice_list", args=[3,1]))
        self.assertEqual(response.status_code, 404)
        # Getting draft invoice with purchase order id 3 which doesn't exist
        response = self.client.get(reverse("invoice:draft_invoice_list", args=[1,3]))
        self.assertEqual(response.status_code, 404)
        purchase_order_info_data ={ 
            "event_id": self.event, 
            "purchase_order_creation_datetime": "2021-05-17T09:49:43.583737Z", 
            "buyer_id": self.buyer, 
            "buyer_purchase_order_id": "123", 
            "buyer_entity_name": "Factwise", 
            "buyer_billing_address_id": self.address1, 
            "buyer_shipping_address_id": self.address1, 
            "buyer_approver_user_id": self.user1, 
        ...
        PurchaseOrder.objects.create(**purchase_order_info_data)
        # Getting draft invoice with purchase order id 2 and user id 2 which is not allowed
        response = self.client.get(reverse("invoice:draft_invoice_list", args=[2,2]))
        self.assertEqual(response.status_code, 400)

    def test_post_valid_invoice(self):
        # posting status invoice with 1
        data = {
            "invoice_info": {
                "invoice_type": "goods",
        ...
        response = self.client.post(reverse("invoice:draft_invoice_list", args=[1,1]), json.dumps(data), content_type="application/json")
    ...
    def test_post_invalid_draft_invoice(self):
        # posting status invoice with 2 which doesn't exist
        data = {
            "invoice_info": {
                "invoice_type": "goods",
                "created_by_user_id": 1,
        ...
        response = self.client.post(reverse("invoice:draft_invoice_list", args=[2,2]), json.dumps(data), content_type="application/json")
        self.assertEqual(response.status_code, 404)
