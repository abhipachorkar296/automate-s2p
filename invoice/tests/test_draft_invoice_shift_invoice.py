...
class DraftInvoiceShiftInvoiceModelViewTest(TestCase):
	...
	def setUp(self):
		...
		DraftInvoiceItemCharge.objects.create(**data_invoice_item_charge_1)
		data_invoice_item_charge_2 = {
			"invoice_line_item_id" : self.invoice_item,
			"charge_name": "CGST",
			"charge_percentage": 5
		}
		DraftInvoiceItemCharge.objects.create(**data_invoice_item_charge_2)
	...
	def test_post_valid_shift_draft_invoice_to_invoice(self):
		# Shifting purchase order with id 1
		response = self.client.post(reverse("invoice:draft_invoice_shift_invoice", args=[1]))
		self.assertEqual(response.status_code, 201)

		# Verifying the new real invoice data after a successfull shift
	...
	def test_post_invalid_shift_draft_invoice_to_invoice(self):
		# Creating a new draft invoice with data that is not valid for a real invoice
		data_invoice_info = {
			"invoice_type": "goods",
			"created_by_user_id": self.user1,
			"purchase_order_id": self.purchase_order,
		...
		DraftInvoice.objects.create(**data_invoice_info)
		self.invoice = DraftInvoice.objects.get(invoice_id=2)
		data_invoice_item = {
			"invoice_id" : self.invoice,
			"purchase_order_line_item_id": self.purchase_order_item,
			"item_id": self.item,
		...
		# Shifting the draft_invoice with id 2 which is invalid to become a real invoice
		response = self.client.post(reverse("invoice:draft_invoice_shift_invoice", args=[2]))
		self.assertEqual(response.status_code, 400)

		# Creating a draft invoice with zero items and trying to shift
		data_invoice_info = {
			"invoice_type": "goods",
			"created_by_user_id": self.user1,
			"seller_invoice_id": "123",
		...
		DraftInvoice.objects.create(**data_invoice_info)
		self.invoice = DraftInvoice.objects.get(invoice_id=3)
		data_invoice_buyer_info = {
			"invoice_id" : self.invoice,
			"buyer_id": self.buyer,
			"identification_id": self.entity_identitfication1,
		...
		# Shifting the draft_invoice with id 3 which is invalid to become a real invoice
		response = self.client.post(reverse("invoice:draft_invoice_shift_invoice", args=[3]))
		self.assertEqual(response.status_code, 400)

		# Shifting the draft_invoice with id 4 which does not exist
		response = self.client.post(reverse("invoice:draft_invoice_shift_invoice", args=[4]))
