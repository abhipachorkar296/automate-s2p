...
class InvoiceModelViewTest(APITestCase):
	...
	def test_get_valid_invoice(self):
		# Getting invoice with id 1
	...
	def test_get_invalid_invoice(self):
		# Getting invoice with id 2 which doesn't exist
		response = self.client.get(reverse("invoice:invoice", args=[2]))
		self.assertEqual(response.status_code, 404)
	
	def test_patch_valid_invoice(self):
		# Patching status invoice with 1
		# Creating a goods receipt so that invoice becomes ongoing from issued
		...
		# Patching with changing seller_invoice_id and rate of invoice_line_item_id 1
		data = {
			"invoice_info": {
				"invoice_id": 1,
				"invoice_type": "goods",
				"created_by_user_id": 1,
				"seller_invoice_id": "1555",
				"provisional_invoice_id": "",
				"delivery_document_id": "",
				"purchase_order_id": 1,
				"buyer_purchase_order_id": "131",
		...
		response = self.client.patch(reverse("invoice:invoice", args=[1]), json.dumps(data), content_type="application/json")
		self.assertEqual(response.status_code, 200)
		invoice = Invoice.objects.get(invoice_id=1)
		invoice_info = InvoiceSerializer(invoice).data
		invoice_item_list = InvoiceItemSerializer(invoice.invoiceitem_set.all(), many=True).data

		self.assertEqual(invoice_info["invoice_id"], 1)
		self.assertEqual(invoice_info["buyer_id"], 1)
		self.assertEqual(invoice_info["seller_id"], 3)
		self.assertEqual(invoice_info["status"], "ongoing")
		self.assertEqual(invoice_info["seller_invoice_id"], "1555")
		
		self.assertEqual(len(invoice_item_list), 1)
		self.assertEqual(invoice_item_list[0]["invoice_line_item_id"], 1)
		self.assertEqual(invoice_item_list[0]["invoice_id"], 1)
		self.assertEqual(invoice_item_list[0]["item_id"], 1)
		self.assertEqual(Decimal(invoice_item_list[0]["rate"]), Decimal(200))
		self.assertEqual(Decimal(invoice_item_list[0]["amount_invoiced"]), Decimal("6750"))
		self.assertEqual(Decimal(invoice_item_list[0]["quantity_invoiced"]), Decimal("30"))
		self.assertEqual(invoice_item_list[0]["currency_code"], "USD")
		
		quantity_accepted = 20
		rate = 200
		all_taxes_percentage_sum = 10
		shipping_per_unit = 5
		invoice_discount_percentage = 0
		expected_amount_due = round(Decimal((1+Decimal(0.01)*all_taxes_percentage_sum-Decimal(0.01)*invoice_discount_percentage)*quantity_accepted*rate + quantity_accepted*shipping_per_unit), DECIMAL_COMPARISON_PRECISION)
		self.assertEqual(Decimal(invoice_item_list[0]["amount_due"]), expected_amount_due)

		# Patching with changing charges of invoice_line_item_id 1
		data = {
			"invoice_info": {
				"invoice_id": 1,
				"invoice_type": "goods",
			...
			"invoice_item_list": [
				{
					"item_info": {
						...
						"rate": 100,
						"quantity_invoiced": 30,
						"shipping_per_unit": 10,
						"amount_invoiced": 3600,
						"amount_due": 0,
						"amount_paid": 0,
						"currency_code": "USD",
						"payment_terms_reference_date_type": "receipt_date",
		...
		self.assertEqual(invoice_item_list[0]["invoice_id"], 1)
		self.assertEqual(invoice_item_list[0]["item_id"], 1)
		self.assertEqual(Decimal(invoice_item_list[0]["rate"]), Decimal(100))
		self.assertEqual(Decimal(invoice_item_list[0]["amount_invoiced"]), Decimal(3600))
		self.assertEqual(Decimal(invoice_item_list[0]["quantity_invoiced"]), Decimal(30))
		self.assertEqual(invoice_item_list[0]["currency_code"], "USD")
		
		quantity_accepted = 20
		rate = 100
		all_taxes_percentage_sum = 15
		shipping_per_unit = 10
		invoice_discount_percentage = 5
		expected_amount_due = round(Decimal((1+Decimal(0.01)*all_taxes_percentage_sum-Decimal(0.01)*invoice_discount_percentage)*quantity_accepted*rate + quantity_accepted*shipping_per_unit), DECIMAL_COMPARISON_PRECISION)
		self.assertEqual(Decimal(invoice_item_list[0]["amount_due"]), expected_amount_due)
	
	def test_patch_invalid_invoice(self):
		...
		response = self.client.patch(reverse("invoice:invoice", args=[2]), json.dumps(data), content_type="application/json")
		self.assertEqual(response.status_code, 404)

		# Patching invoice without any Goods Receipt present against it
		data = {
			"invoice_info": {
				"invoice_id": 1,
				"invoice_type": "goods",
		...
		# Patching invoice with a Goods Receipt present against it but having blank seller_invoice_id
		data = {
			"invoice_info": {
				"invoice_id": 1,
				"invoice_type": "goods",
		...
		# Patching invoice with invoice_items not prsent in this invoice
		data = {
			"invoice_info": {
				"invoice_id": 1,
				"invoice_type": "goods",
		...
		# Patching invoice with an invoice_item missing (basically trying to delete that)
		# which should not be allowed as during Patching a provisional invoice, no items 
		# can be deleted
		data = {
			"invoice_info": {
				"invoice_id": 1,
		...
		# Patching with changing invoiced_quantity of invoice_line_item_id 1
		data = {
			"invoice_info": {
				"invoice_id": 1,
				"invoice_type": "goods",
		...
		# Checking if a Payment is present against invoice

		# Checking Transaction Working when
		# All Pre Checks have passed but the data which is going to be saved is not valid
		# For that, here only invalid seller information is being given invalid with not 
		# existing identification_id
			...
			"invoice_seller_information": [
				{
					"invoice_id": 1,
					"seller_id": 3,
					"identification_id": 4, # This id doesn't exist
					"identification_name": "CGST",
					"identification_value": "1234557"
				}
			]
		...
		# Verifying that the original data has not been altered
		invoice = Invoice.objects.get(invoice_id=1)
		invoice_info = InvoiceSerializer(invoice).data
		...
		for item in invoice_items:
			item_data = {}
			item_data["item_info"] = InvoiceItemSerializer(item).data
			item_data["item_charge"] = InvoiceItemChargeSerializer(item.invoiceitemcharge_set.all(), many=True).data
			item_data["item_attribute"] = InvoiceItemAttributeSerializer(item.invoiceitemattribute_set.all(), many=True).data
			invoice_item_list.append(item_data)
		invoice_buyer_information = InvoiceBuyerInformationSerializer(invoice.invoicebuyerinformation_set.all(), many=True).data
		invoice_seller_information = InvoiceSellerInformationSerializer(invoice.invoicesellerinformation_set.all(), many=True).data
	...
	def test_delete_valid_invoice(self):
		# Deleting a valid invoice with id 1
		response = self.client.delete(reverse("invoice:invoice", args=[1]))
		self.assertEqual(response.status_code, 200)
	...
	def test_delete_invalid_invoice(self):
		...
		response = self.client.delete(reverse("invoice:invoice", args=[1]))
		self.assertEqual(response.status_code, 200)
		# Again deleting the invoice (already deleted) with id 1
		response = self.client.delete(reverse("invoice:invoice", args=[1]))
		self.assertEqual(response.status_code, 400)

		# Deleting a valid invoice with id 2 which does not exist
		response = self.client.delete(reverse("invoice:invoice", args=[2]))
		self.assertEqual(response.status_code, 404)
	
	def test_get_valid_invoice_status(self):
		# Getting invoice status with invoice_line_item_id 1
		response = self.client.get(reverse("invoice:invoice_status", args=[1]))
		self.assertEqual(response.status_code, 200)
		self.assertEqual(response.data["status"], "issued")

		# Creating a goods invoice to change the status of the invoice
		invoice = Invoice.objects.get(invoice_id=1)
		invoice_line_item = InvoiceItem.objects.get(invoice_line_item_id=1)
		goods_receipt_info = {
			"invoice_line_item_id": invoice_line_item,
	...
	def test_get_invalid_invoice_status(self):
		# Getting invoice status with invoice_line_item_id 2 which does not exist
		response = self.client.get(reverse("invoice:invoice_status", args=[2]))
		self.assertEqual(response.status_code, 404)