...
class PaymentModelViewTest(APITestCase):
    ...
    def test_get_valid_payment(self):
        # Getting payment with id 1
        response = self.client.get(reverse("payment:payment", args=[1]))
    ...
    def test_get_invalid_payment(self):
        # Getting payment with id 2 which doesn't exist
        response = self.client.get(reverse("payment:payment", args=[2]))
        self.assertEqual(response.status_code, 404)

    def test_get_valid_payment_from_invoice(self):
        # Getting payment_from_invoice with id 1
        response = self.client.get(reverse("payment:payment_from_invoice", args=[1]))
        self.assertEqual(response.status_code, 200)
        payment_data = response.data
        
        self.assertEqual(payment_data[0]["payment_id"], 1)
        self.assertEqual(payment_data[0]["created_by_user_id"], 1)
        self.assertEqual(payment_data[0]["from_entity_id"], 1)
        self.assertEqual(payment_data[0]["to_entity_id"], 3)
        self.assertEqual(payment_data[0]["payment_mode"], "ll")
        self.assertEqual(payment_data[0]["total_amount"], '2.0200000000')
        
    def test_get_invalid_payment_from_invoice(self):
        # Getting payment_from_invoice with id 2 which doesn't exist
        response = self.client.get(reverse("payment:payment_from_invoice", args=[2]))
        self.assertEqual(response.status_code, 404)