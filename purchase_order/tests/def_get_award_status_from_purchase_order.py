...
class AwardStatusFromPurchaseOrderViewTest(TestCase):
    ...
    def setUp(self):
        ...
        entity_3_default_identification_data = {
            "entity_id": self.entity3,
            "identification_id": self.entity_identitfication2
        }
        EntityPurchaseOrderDefaultIdentification.objects.create(**entity_3_default_identification_data)
    ...
    def test_get_valid_award_status_from_purchase_order(self):
        # Getting award status from purchase order with id 1
        response = self.client.get(reverse("purchase_order:award_status_from_purchase_order", args=[1]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["deal_status"], "Deal Accepted")
    
    def test_get_invalid_award_status_from_purchase_order(self):
        # Getting award status from purchase order with id 2 which does not exist
        response = self.client.get(reverse("purchase_order:award_status_from_purchase_order", args=[2]))
