...
class DraftPurchaseOrderViewTest(TestCase):
    def setUp(self):
        '''
        Populating test db
        '''
    ...
    def test_get_valid_purchase_order(self):
        ...
        '''
        response = self.client.get(reverse("purchase_order:draft_purchase_order" , args=[1]))
    ...
    def test_get_invalid_purchase_order(self):
        '''
        Getting purchase order with ID 2, which does not exist
        '''
        response = self.client.get(reverse("purchase_order:draft_purchase_order" , args=[2]))
        self.assertEqual(response.status_code, 404)

    def test_patch_valid_purchase_order(self):
        '''
        Patching purchase order with ID 1 with status
        '''
        data = {
            "purchase_order_info": {
                "event_id": 1, 
                ...
                "buyer_contact_phone": "9xxxxxx12", 
                "buyer_contact_email": "a@gmail.com", 
                "is_freight_purchase_order": "False", 
                "seller_id": 3, 
                "seller_entity_name": "a", 
                "seller_address_id": 2, 
                "seller_contact_user_id": 2, 
                "seller_contact_name": "a", 
                "seller_contact_phone": "9xxxxx12", 
                "seller_contact_email": "apple1@gmail.com", 
        ...
        response = self.client.patch(reverse("purchase_order:draft_purchase_order" , args=[1]), data, content_type='application/json')
        self.assertEqual(response.status_code, 201)
    
    def test_patch_invalid_purchase_order(self):
        '''
        Patching purchase order with ID 2 with stauts
        '''
        data = {
            "purchase_order_info": {
                "purchase_order_id": 1, 
        ...
        data = json.dumps(data)
        response = self.client.patch(reverse("purchase_order:draft_purchase_order" , args=[2]), data, content_type='application/json')
        self.assertEqual(response.status_code, 404)
    
    def test_invalid_deleted_purchase_order(self):
        '''
        Patching deleted purchase order
        '''
        response = self.client.delete(reverse("purchase_order:draft_purchase_order" , args=[1]))
        response = self.client.patch(reverse("purchase_order:draft_purchase_order" , args=[1]))
        self.assertEqual(response.status_code, 404)
    
    def test_delete_valid_purchase_order(self):
        # Deleting purchase order with ID 1
        response = self.client.delete(reverse("purchase_order:draft_purchase_order" , args=[1]))
        self.assertEqual(response.status_code, 200)
    
    def test_delete_invalid_purchase_order(self):
        # Deleting purchase order with id 2 which does not exist
        response = self.client.delete(reverse("purchase_order:draft_purchase_order" , args=[2]))
        self.assertEqual(response.status_code, 404)