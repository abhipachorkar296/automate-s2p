...
class DraftPurchaseOrderListViewTest(TestCase):
    ...
    def test_get_valid_purchase_order_list(self):
        '''
        Getting purchase order with event id  1
        '''
        response = self.client.get(reverse("purchase_order:purchase_order_list" , args=[1]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data[0]["purchase_order_info"]["purchase_order_id"], 1)
        self.assertEqual(response.data[0]["purchase_order_info"]["buyer_purchase_order_id"], "1233")
        self.assertEqual(response.data[0]["purchase_order_item_list"][0]["item_info"]["purchase_order_line_item_id"], 1)
        self.assertEqual(response.data[0]["purchase_order_item_list"][0]["item_attribute"][0]["attribute_id"], 1)
        self.assertEqual(response.data[0]["purchase_order_item_list"][0]["item_charge"][0]["charge_name"], "GST")

    def test_get_invalid_purchase_order_list(self):
        '''
        Getting purchase order with event id  1
        '''
        response = self.client.get(reverse("purchase_order:purchase_order_list" , args=[2]))
        self.assertEqual(response.status_code, 404)
    
    def test_get_valid_latest_purchase_order(self):
        '''
        Getting purchase order with event id  1 and seller id 3
        '''
        response = self.client.get(reverse("purchase_order:lastest_purchase_order" , args=[1,3]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_invalid_latest_purchase_order(self):
        '''
        Getting purchase order with event id  2(non-existent) and seller id 3
        '''
        response = self.client.get(reverse("purchase_order:lastest_purchase_order" , args=[2,3]))
        self.assertEqual(response.status_code, 404)
        '''
        Getting purchase order with event id  1 and seller id 2(non-existent)
        '''
        response = self.client.get(reverse("purchase_order:lastest_purchase_order" , args=[2,3]))
        self.assertEqual(response.status_code, 404)

    def test_patch_valid_rescind_purchase_order_by_buyer(self):    
        '''
        patchting purchase order with user id  1  and purchase order id 1
        '''
        response = self.client.patch(reverse("purchase_order:rescind_purchase_order" , args=[1,1]))
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(response.data["purchase_order_info"]["status"], "rescind")


    def test_patch_invalid_rescind_purchase_order(self):
        '''
        patchting purchase order with user id  2 and purchase order id 1(no relation between user and purchase order)
        '''
        response = self.client.patch(reverse("purchase_order:rescind_purchase_order" , args=[2,1]))
        self.assertEqual(response.status_code, 404)
        '''
        patchting purchase order with user id  1 and  purchase order id 2(non-existent)
        '''
        response = self.client.patch(reverse("purchase_order:rescind_purchase_order" , args=[1,2]))
        self.assertEqual(response.status_code, 404)
        '''
        patchting purchase order with user id 3(non-existent) and  purchase order id 1
        '''
        response = self.client.patch(reverse("purchase_order:rescind_purchase_order" , args=[3,1]))
        self.assertEqual(response.status_code, 404)

    def test_get_valid_event_purchase_order_list(self):
        '''
        Getting purchase order with event id  1
        '''
        response = self.client.get(reverse("purchase_order:event_purchase_order_list" , args=[1]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data[0]["purchase_order_info"]["purchase_order_id"], 1)
        self.assertEqual(response.data[0]["purchase_order_info"]["buyer_purchase_order_id"], "1233")
        self.assertEqual(response.data[0]["purchase_order_item_list"][0]["item_info"]["purchase_order_line_item_id"], 1)
    ...
    def test_get_invalid_event_purchase_order_list(self):
        '''
        Getting purchase order with event id  2(non existent)
        '''
        response = self.client.get(reverse("purchase_order:event_purchase_order_list" , args=[2]))
        self.assertEqual(response.status_code, 404)