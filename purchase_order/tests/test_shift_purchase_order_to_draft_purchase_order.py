...
class ShiftPurchaseOrderDraftPurchaseOrderModelViewTest(TestCase):
    ...
    def test_post_valid_shift_purchase_order_to_draft_purchase_order(self):
        # Shifting purchase order with id 1
        response = self.client.post(reverse("purchase_order:shift_purchase_order_to_draft_purchase_order", args=[1]))
        ...
        self.assertEqual(purchase_order_info["status"], "draft")
        ...
        self.assertIsNone(purchase_order_info["seller_acknowledgement_user_id"])
        self.assertIsNone(purchase_order_info["seller_acknowledgement_datetime"])
        self.assertIsNone(purchase_order_info["closing_user_id_seller"])
        self.assertEqual(purchase_order_info["closing_comment_seller"], "")
        self.assertIsNone(purchase_order_info["closing_datetime_seller"])
        self.assertIsNone(purchase_order_info["closing_user_id_buyer"])
        self.assertEqual(purchase_order_info["closing_comment_buyer"], "")
        self.assertIsNone(purchase_order_info["closing_datetime_buyer"])
        self.assertIsNone(purchase_order_info["purchase_order_close_datetime"])

    def test_post_invalid_shift_purchase_order_to_draft_purchase_order(self):
        # Shifting purchase order with id 2 which does not exist
        response = self.client.post(reverse("purchase_order:shift_purchase_order_to_draft_purchase_order", args=[2]))
