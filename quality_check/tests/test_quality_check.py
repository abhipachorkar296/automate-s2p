...
class QualityCheckModelViewTest(APITestCase):
	...
	def setUp(self):
		...
		data = {
			"goods_receipt_entry_id": goods_receipt,
			"created_by_user_id": self.user1,
	...
	def test_get_valid_quality_check(self):
		# Getting quality_check with id 1
		response = self.client.get(reverse("quality_check:quality_check", args=[1]))
		self.assertEqual(response.status_code, 200)
		data = response.data["quality_check_info"]
	...
	def test_get_invalid_quality_check(self):
		# Getting quality_check with id 2 which does not exist
		response = self.client.get(reverse("quality_check:quality_check", args=[2]))
