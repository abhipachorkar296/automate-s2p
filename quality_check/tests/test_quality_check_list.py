...
class QualityCheckListModelViewTest(APITestCase):
	...
	def setUp(self):
		...
		self.goods_receipt = GoodsReceipt.objects.create(**goods_receipt_info)
		...
		invoice_line_item.receipt_date = self.goods_receipt.receipt_datetime
		...
		data = {
			"goods_receipt_entry_id": self.goods_receipt,
			"created_by_user_id": self.user1,
		...
		quality_check = QualityCheck.objects.create(**data)

	def test_get_valid_quality_check_list(self):
		# Getting quality_check list with goods_receipt_entry id 1
		response = self.client.get(reverse("quality_check:quality_check_list", args=[1]))
		self.assertEqual(response.status_code, 200)
		# one quality_check is expected here
		self.assertEqual(len(response.data), 1)
		# Checking the first quality_check
		data = response.data[0]["quality_check_info"]
		self.assertEqual(data["quality_check_id"], 1)
		self.assertEqual(data["goods_receipt_entry_id"], 1)
		self.assertEqual(data["created_by_user_id"], 1)
		self.assertEqual(data["quality_check_reason"], "Tested")
		self.assertEqual(Decimal(data["quantity_rejected"]), Decimal("0"))

		# Creating a second quality check
		data = {
			"goods_receipt_entry_id": self.goods_receipt,
			"created_by_user_id": self.user1,
			"created_by_name": "Matt Parr",
		...
		response = self.client.get(reverse("quality_check:quality_check_list", args=[1]))
		self.assertEqual(response.status_code, 200)
		# two quality_checks are expected here
		self.assertEqual(len(response.data), 2)
		# Sorting the data according to the quality_check_id to test the data
		response.data.sort(key=lambda data: data["quality_check_info"]["quality_check_id"])
		# Checking the first quality_check
		data = response.data[0]["quality_check_info"]
		self.assertEqual(data["quality_check_id"], 1)
		self.assertEqual(data["goods_receipt_entry_id"], 1)
		...
		# Checking the second quality_check
		data = response.data[1]["quality_check_info"]
		self.assertEqual(data["quality_check_id"], 2)
		self.assertEqual(data["goods_receipt_entry_id"], 1)
		self.assertEqual(data["created_by_user_id"], 1)
		self.assertEqual(data["quality_check_reason"], "Tested")
		self.assertEqual(Decimal(data["quantity_rejected"]), Decimal("0"))

	def test_get_invalid_quality_check_list(self):
		# Getting quality_check list with goods_receipt_entry id 2 which does not exist
		response = self.client.get(reverse("quality_check:quality_check_list", args=[2]))
		self.assertEqual(response.status_code, 404)
	
	def test_post_valid_quality_check_list(self):
		# Posting a quality_check with goods_receipt_entry_id 1
		invoice_line_item = InvoiceItem.objects.get(invoice_line_item_id=1)
		original_amount_due = round(invoice_line_item.amount_due, DECIMAL_COMPARISON_PRECISION)
		data = {
        "quality_check_info":{
				"goods_receipt_entry_id": 1,
		...
		response = self.client.post(reverse("quality_check:quality_check_list", args=[1]), json.dumps(data), content_type="application/json")
		self.assertEqual(response.status_code, 201)
		
		# Checking for the posted data
		data = QualityCheckSerializer(QualityCheck.objects.get(quality_check_id=2)).data
		self.assertEqual(data["quality_check_id"], 2)
		self.assertEqual(data["goods_receipt_entry_id"], 1)
		self.assertEqual(data["created_by_user_id"], 1)
		self.assertEqual(data["quality_check_reason"], "Tested")
		self.assertEqual(Decimal(data["quantity_rejected"]), Decimal("10"))
		...
		shipping_per_unit = invoice_line_item.shipping_per_unit
		invoice_discount_percentage = invoice.invoice_discount_percentage
		rejected_amount = round(Decimal((1+Decimal(0.01)*all_taxes_percentage_sum-Decimal(0.01)*invoice_discount_percentage)*quantity_rejected*rate + quantity_rejected*shipping_per_unit), DECIMAL_COMPARISON_PRECISION)

		# Checking the decreased amount_due
		self.assertEqual(round(original_amount_due-rejected_amount, DECIMAL_COMPARISON_PRECISION), round(invoice_line_item.amount_due, DECIMAL_COMPARISON_PRECISION))
	
	def test_post_invalid_quality_check_list(self):
		# Posting a quality_check with goods_receipt_entry_id 2 which does not exist
		data = {
			"quality_check_info":{
				"goods_receipt_entry_id": 2,
				"created_by_user_id": 1,
				"created_by_name": "Matt Parr",
				"created_by_phone": "91xxxx11",
				"created_by_email": "mattparr@factwise.io",
		...
		response = self.client.post(reverse("quality_check:quality_check_list", args=[2]), json.dumps(data), content_type="application/json")
		self.assertEqual(response.status_code, 404)
		
		# Posting a quality_check invalid format data with goods_receipt_entry_id 1 
		data = {
			"goods_receipt_entry_id": 1,
			"created_by_user_id": 1,
			"created_by_name": "Matt Parr",
		...
		# Posting a quality_check with more rejections than accepted
		data = {
			"quality_check_info":{
				"goods_receipt_entry_id": 1,
				"created_by_user_id": 1,
		...
		# Posting a quality_check with incomplete data
		data = {
			"quality_check_info":{
				"goods_receipt_entry_id": 2,
				"quantity_rejected": 10
