...
def post_award_version(award_id=1):
    base_url = "http://127.0.0.1:8000/event"
    endpoint = "/award_new_version/"+str(award_id)
    ...
    response = requests.post(abs_url, data=json.dumps(award_version), headers=headers)
    print(response.status_code)

post_award_version(1)